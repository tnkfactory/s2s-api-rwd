...
- 2026/04/16
  - 2.9 광고 적립 URL 호출 전송 파라미터 누락분 추가 (uid, multi_yn)
- 2026/02/11
  - 2.5 queryjoin 반환 값(list에 데이터 추가 - order : 캠페인 순서, pay_yn : 캠페인 달성 여부)
- 2025/06/11
  - 3.3 광고유형코드(415 : 쿠팡(복권형)) 추가
- 2025/05/30
  - 3.1 리턴코드(E12 : CPS 하루 구매 건수 초과) 추가
- 2025/04/10
  - 3.3 광고유형코드(116 : 저장형(플레이스)) 추가
- 2025/03/27
...
|Optional 파라메터|(목록 조회 필터)|타입|
| :-: | :-- | :--: |
|ad\_type|0 : 일반광고, 1 : CPS(쇼핑광고)<br />해당 값이 있어야 목록 조회 필터가 적용됩니다.|Integer|
|cmpn\_types|ad\_type : 0 - 3.3 광고 유형<br />ad\_type : 1 - 3.4.2 카테고리 구매형<br />,(콤마)구분자로 포함하고자 하는 유형 및 카테고리를 추가합니다.|String|
|limit|목록 조회 건수|Integer|
...
| :-: | :-- |
|app\_id|**광고앱 고유 ID**|
|actn\_id|<p>광고 형태 값을 나타냄</p><p>- 0 : 설치형 (설치형의 경우 매체에서 설치확인을 하고 “2.6 requestReward”를 사용하여 적립요청을 해야함)</p><p>- 1 : 실행형</p><p>- 2 : 액션형</p><p>실행형과 액션형은 Tnk 서버에서 매체서버로 적립내용을 호출해준다.</p><p>- 4 : 클릭형 또는 다중참여형</p><p>TNK서버에서 매체서버로 적립내용 호출해준다.(동일 광고 ID에 한 사용자가 여러번 참여 가능)</p><p>- 5 : 구매형</p><p>사용자 결제완료시 매체서버로 적립내용을 호출해준다.</p>|
...
|pay\_dt|실적 확정 일시.(포인트 지급 조건(실적 확정)이 충족된 시간) – milliseconds (ex - 1580358657348)<br />포인트 지연 지급 캠페인 / 첫 포스트백 에러 후 재전송 등의 경우에는 실적 확정 일시와 포스트백 받은 시간의 차이가 발생 할 수 있다.|long|
|pay\_cnt|구매형 – 구매수량|int|
|actn\_id|<p>- 0 : 설치형</p><p>- 1 : 실행형</p><p>- 2 : 액션형</p><p>- 4 : 클릭형 또는 다중참여형</p><p>- 5 : 구매형</p><p>- 6 : 이벤트</p>|int|
|uid|캠페인 참여 시 전달된 adid(idfa) 값 - 소문자, - 제거된 값|string(32)|
|multi\_yn|멀티캠페인 여부 (Y : 멀티캠페인, N : 싱글캠페인)|string(1)|
